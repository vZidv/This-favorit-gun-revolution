...
public class Weapon_Enemy : MonoBehaviour
{
    ...
    public enum BossType {ItIsNotBoss,Boss_1,Boss_2}
    ...
    void Update()
    {
        ...
        else if (bossType == BossType.Boss_2)
        {
            if (timeBeforFire <= 0)
            {
                if (timeforFire <= 0)
                {
                    ...
                    Quaternion rotateZ ;
                    float scatter = 30;
                    float rand = Random.Range(-scatter, scatter);
                    Instantiate(BulletPrefab, transform.position, rotateZ = Quaternion.Euler(0, 0, rotate + rand));
                    timeforFire = timeforFireStart;
                }
                else
                {
